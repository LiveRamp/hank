...
public class Daemon implements RingGroupChangeListener, DomainGroupChangeListener {
  ...
  private void processUpdates() {
    ...
    if (snapshotRingGroupConfig.isUpdating()) {
      ...
      for (RingConfig ring : snapshotRingGroupConfig.getRingConfigs()) {
        if (ring.isUpdatePending()) {
          anyUpdatesPending = true;

          switch (ring.getState()) {
            case AVAILABLE:
              if (ring.getUpdatingToVersionNumber() == ring.getVersionNumber()) {
            ...
            case IDLE:
              if (getOldestHostVersion(ring) == ring.getUpdatingToVersionNumber()) {
      ...
      if (!anyUpdatesPending) {
        ringGroupConfig.updateComplete();
      }
    }
  ...
  private int getOldestHostVersion(RingConfig ring) {
