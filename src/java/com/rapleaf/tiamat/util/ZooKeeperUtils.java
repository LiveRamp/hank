 ...
 */
package com.rapleaf.tiamat.util;
...
public class ZooKeeperUtils {
  ...
  private static final String RING_PATH_FORMAT = RING_GROUP_ROOT + "/%s/ring-%03d";
  ...
  private static final String DOMAIN_GROUP_PATH_FORMAT = DOMAIN_GROUP_ROOT + "/%s";
  ...
  public static void checkExists(ZooKeeper zk, String path) throws DataNotFoundException, InterruptedException {
    checkExists(zk, path, "Node does not exist at " + path);
  }
  
  public static void checkExists(ZooKeeper zk, String path, String message) throws DataNotFoundException, InterruptedException {
    try {
      ...
      // bliu: I have no idea what exception can be thrown (not documented), so I'll just die so that we can find out!
      throw new RuntimeException("The code does not expect an exception here.", e);
    }
  }
  public static void checkExistsOrDie(ZooKeeper zk, String path) throws InterruptedException {
    checkExistsOrDie(zk, path, "Fatal: Node does not exist at " + path);
  }
  public static void checkExistsOrDie(ZooKeeper zk, String path, String message) throws InterruptedException {
    try {
      if (zk.exists(path, null) == null) {
        LOG.fatal(message);
  ...
  public static List<String> getChildrenOrDie(ZooKeeper zk, String path) throws InterruptedException {
    try {
      return zk.getChildren(path, null);
    }
    catch (KeeperException e) {
      // The only time we should get this exception is when the node does not exist
      String dieMessage = "Node does not exist at " + path;
      LOG.fatal(dieMessage, e);
      throw new RuntimeException(dieMessage, e);
    }
  }
  public static String getStringOrDie(ZooKeeper zk, String path) throws InterruptedException {
    try {
      return Bytes.bytesToString(zk.getData(path, null, null));
    }
    catch (KeeperException e) {
      // The only time a KeeperException is thrown from ZooKeeper#getData is when the node doesn't exist.
      String dieMessage = "Node does not exist at " + path;
      LOG.fatal(dieMessage, e);
      throw new RuntimeException(dieMessage, e);
    }
  ...
  public static int getIntOrDie(ZooKeeper zk, String path) throws InterruptedException {
    try {
  ...
  public static Stat setDataOrDie(ZooKeeper zk, String path, byte[] data) throws InterruptedException {
    try {
      return zk.setData(path, data, -1);
    }
    catch (KeeperException e) {
      // The only time we should get a KeeperException is if the node doesn't exist
  ...
  public static void createNodeOrFailSilently(ZooKeeper zk, String path) throws InterruptedException {
    setDataOrFailSilently(zk, path, null);
  }
  
  public static void setDataOrFailSilently(ZooKeeper zk, String path, byte[] data) throws InterruptedException {
    try {
      if (zk.exists(path, null) != null) {
  ...
  public static void createNodeRecursively(ZooKeeper zk, String path) 
    ...
    try {
      ...
      createNodeRecursively(zk, path); // Potential for infinite loop, but unlikely
    }
    catch (KeeperException e) {
      LOG.warn(e);
    }
  ...
  public static void rmrf(ZooKeeper zk, String path) throws InterruptedException, KeeperException {
